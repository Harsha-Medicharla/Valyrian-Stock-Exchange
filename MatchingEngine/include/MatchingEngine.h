...
private:
  ...
  void pushFillOrderEvent(const Order *o, Price fill_price, Qty fill_qty,
                          Qty remaining_after, OrderState st) noexcept;

  void pushCancelAck(OrderId oid, UserId uid, OrderState st, Qty rem) noexcept;

  void pushModifyAck(OrderId oid, UserId uid, OrderType ty, Side sd, Price px, Qty qty,
                     Qty rem, OrderState st) noexcept;
...
public:
  ...
  bool onCancelOrder(OrderId order_id)
  {
    ...
    try
    {
      ...
      pushCancelAck(order_id, uid, st, rem);
      pushDbCancel(order_id, uid, sd, ty, px, q, rem);
      ...
      if (new_price == order->price && new_qty < order->quantity)
      {
        ...
        else
        {
          pushModifyAck(order_id, order->user_id, order->type, order->side, order->price,
                        order->quantity, order->remaining, order->state);
        ...
        if (success)
        {
          ...
          if (placed)
          {
            pushModifyAck(order_id, placed->user_id, placed->type, placed->side, placed->price,
                          placed->quantity, placed->remaining, placed->state);
                                                ...
                                                Price trade_price, Qty traded) noexcept
{
  ...
  if (dbQueue_ && !isRecovering_)
  {
    ...
    da.fill_qty = traded;
    da.timestamp = getCurrentWallTime();
    ...
    dr.fill_qty = traded;
    dr.timestamp = getCurrentWallTime();
  ...
  if (tradeQueue_ && !isRecovering_)
  {
    ...
    te.timestamp = getCurrentWallTime();
...
inline void MatchingEngine::pushModifyAck(OrderId oid, UserId uid, OrderType ty, Side sd, Price px,
                                          Qty qty, Qty rem, OrderState st) noexcept
{
  if (!orderQueue_ || isRecovering_)
    return;
  OrderEvent ev{};
                                         ...
                                         Qty qty, Qty rem) noexcept
{
  ...
  de.qty = qty;
  de.remaining = rem;
  de.timestamp = getCurrentWallTime();
...
inline void MatchingEngine::match(Order *incoming_order) noexcept
{
  ...
  while (incoming_order->remaining > 0)
  {
    ...
    emitPostTradeEvents(incoming_order, resting_snap, best_price, traded);
  }
}
